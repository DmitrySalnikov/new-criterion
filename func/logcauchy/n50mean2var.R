...
second <- 1+2*par
res <- vector()
for(x in par) res <- rbind(res, Power(t(replicate(M, c(rlogcauchy(n,first[1],second[1]), rlogcauchy(n,x,1+2*x)))), exact = FALSE, logcauchy = TRUE))
res

# store

rownames(res) <- par
rname <- paste0('mean2varN',n,'M',M,'D',D)
