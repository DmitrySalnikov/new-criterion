...
ress <- '/home/d/Study/publication2019/res/student/mean3/'
tables <- '/home/d/Study/publication2019/tables/student/mean3'
...
# n=5 exact
{
  ...
  res <- vector()
  for(x in par) {
    res <- rbind(res, Power(t(replicate(M, c(rt(n,df = first,ncp = par[1]), rt(n,first,x)))), exact = TRUE))
  }
  rownames(res) <- par
...
c1 <- c(c2[1], rep('',4))
{
  ...
  res <- vector()
  for(x in par) {
    res <- rbind(res, PowerAll(t(replicate(M, c(rt(n,first,par[1]), rt(n,first,x))))))
  }
  rownames(res) <- par
