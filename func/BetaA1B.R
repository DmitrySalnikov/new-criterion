...
ress <- '/home/d/Study/publication2019/res/beta/a1b/'
tables <- '/home/d/Study/publication2019/tables/beta/a1b/'
...
# n=5 exact
{
  ...
  res <- vector()
  for(x in par) {
    res <- rbind(res, Power(t(replicate(M, c(rbeta(n,first,par[1]), rbeta(n,first,x)))), exact = TRUE))
  }
  rownames(res) <- par
...
c1 <- c(c2[1], rep('',4))
{
  ...
  res <- vector()
  for(x in par) {
    res <- rbind(res, Power(t(replicate(M, c(rbeta(n,first,par[1]), rbeta(n,first,x))))))
  }
  rownames(res) <- par
