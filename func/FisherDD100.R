...
ress <- '/home/d/Study/publication2019/res/fisher/dd100/'
tables <- '/home/d/Study/publication2019/tables/fisher/dd100/'
...
# n=5 exact
{
  ...
  res <- vector()
  for(x in par) {
    res <- rbind(res, PowerAll(t(replicate(M, c(rf(n,par[1],second), rf(n,x,second)))), exact = TRUE))
  }
  rownames(res) <- par
...
c1 <- c(c2[1], rep('',4))
{
  ...
  res <- vector()
  for(x in par) {
    res <- rbind(res, PowerAll(t(replicate(M, c(rf(n,par[1],second), rf(n,x,second))))))
  }
  rownames(res) <- par
