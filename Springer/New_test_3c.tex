...
\institute{St. Petersburg State University, Department of Mathematics, Russia, Corresponding author \\
    \email{vbmelas@yandex.ru}
    \and
...
We would like to have a test that is appropriate for the case where the  basic distribution belongs to a rather general class of distributions and the alternative distribution  differ only by shift and scale transformation.
...
The test suggested above can be considered as an approximation of logarithm of this ratio for the Cauchy distribution. We suppose tha it will be very efficient for all distributions with property (3).
...
where  F is an arbitrary distribution function with property   (3) and
...
$$
tends to normal random variable with zero expectation and a finite variance.
...
|X_{i}| \leq C$ and
 $\tilde{X_i}=C$ if $X_{i}>0$,
  $\tilde{X_i}=-C$ if $X_{i}<0$ otherwise. And $\tilde{Y_{i}}$ are determined similarly. Note that  $0 \leq \ln (1+x^2) \leq x^2$. Therefore there exists a value $t$ that depends on $\tilde{X}$ and $\tilde{Y}$ such that
\begin{equation}
...
$$
Therefore we obtain for the right hand side of (8) with some algebra
